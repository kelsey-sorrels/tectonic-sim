...
{
  if (plateCache[(int)x][(int)y] != null)
  {
    return plateCache[(int)x][(int)y];
  }
  ...
  for (int i : is)
  {
    for (int j : js)
    {
      ...
      else
      {
        ...
        for (int l = 5; l > 0; l--)
        {
          float r = (float)(0.00005 * width * poissonRandom(5));
          float theta = random(0, (float) (2 * Math.PI));
          PVector rd = dest(i, j, r, theta);
...
void setup()
{
  ...
  plateCache = new Plate[width][height];
